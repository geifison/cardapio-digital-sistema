...
import { useOrdersInit, useOrdersData, useOrdersActions } from "@/stores/ordersStore";

// ----- Helpers -----
function todayISO() {
  // Usar data local atual
  const d = new Date();
...
// Parser robusto para datas vindas do backend (ex.: "YYYY-MM-DD HH:mm:ss")
function parseOrderDate(input?: string | null): Date | null {
  ...
  // Formato SQL sem timezone: tratar como hora local
  if (/^\d{4}-\d{2}-\d{2} \d{2}:\d{2}:\d{2}/.test(s)) {
    const transformed = s.replace(" ", "T");
    const d = new Date(transformed);
    return isNaN(d.getTime()) ? null : d;
  }
  // Fallback
  const d = new Date(s);
...
function getColumnKey(o: Order): ColumnKey {
  if (o.status === "novo") return "novos";
  if (o.status === "aceito" || o.status === "producao") return "producao";
  if (o.status === "entrega") return "entrega";
  if (o.status === "finalizado" || o.status === "cancelado") return "finalizados";
  return "novos"; // fallback
}
...
function SortableCard({ order, disabled, onDetails, onCancel, onAdvance, currentTime, onPrint, busy, onConfirmPayment }: { order: Order; disabled?: boolean; onDetails: () => void; onCancel: (order: Order) => void; onAdvance: (order: Order) => void; currentTime: number; onPrint: (order: Order, type: "kitchen" | "customer") => void; busy?: boolean; onConfirmPayment: (order: Order) => void }) {
  ...
  if (order.status === "producao") {
    ...
    const d = parseOrderDate(tsStr || undefined);
    if (d) statusTs = d.getTime();
  }
...
export default function OrdersPage() {
  console.log('🏠🏠🏠 ORDERS PAGE RENDERIZADO 🏠🏠🏠');
  const { toast } = useToast();
  const today = useMemo(() => todayISO(), []);
  // Inicializa Socket/Listeners sem inscrever o componente no store (evita re-renders globais)
  useOrdersInit();
  // Seletores finos: apenas os campos necessários
  const { orders, loading, error } = useOrdersData();
  // Ações desacopladas do estado para estabilidade de referência
  const { fetchOrders, updateOrderStatus, cancelOrder, updateOrder } = useOrdersActions();
   
  // Logs de debug removidos para melhor performance
  
  // Limites do dia (local) para filtro
  ...
  // Filtrar pedidos por data de hoje
  const todayOrders = useMemo(() => {
    const { start, end } = dayBounds;
    return orders.filter(order => {
      const d = parseOrderDate(order.created_at || undefined);
      if (!d) return false;
      return d >= start && d <= end;
    });
  }, [orders, dayBounds]);
  
  // Mapa de próximo status (mantido para compatibilidade)
  const nextStatusMap = useMemo(() => ({
    novo: 'aceito' as OrderStatus,
    aceito: 'producao' as OrderStatus,
    producao: 'entrega' as OrderStatus,
    entrega: 'finalizado' as OrderStatus,
    finalizado: 'finalizado' as OrderStatus,
    cancelado: 'cancelado' as OrderStatus
  }), []);
  ...
  const selectedOrderFresh = useMemo(() => {
    ...
    return todayOrders.find((o) => o.id === selectedOrder.id) || selectedOrder;
  }, [todayOrders, selectedOrder]);
  ...
  const generateCustomerReceiptHtml = useCallback((order: Order) => {
    const createdAtDate = parseOrderDate(order.created_at) || new Date();
    const createdAt = createdAtDate.toLocaleString("pt-BR");
  ...
  const generateKitchenTicketHtml = useCallback((order: Order) => {
    const createdAtDate = parseOrderDate(order.created_at) || new Date();
    const createdAt = createdAtDate.toLocaleString('pt-BR', { hour: '2-digit', minute: '2-digit' });
    const logoUrl = companyLogoUrl || "";
  ...
  const lastOrdersRef = useRef<Order[]>(todayOrders);
  useEffect(() => {
    if (!selectedOrder) {
      lastOrdersRef.current = todayOrders;
    }
  }, [todayOrders, selectedOrder]);

  const ordersForGrid = selectedOrder ? lastOrdersRef.current : todayOrders;
  ...
  const columns = useMemo(() => {
    ...
    ordersForGrid.forEach((o) => {
      const columnKey = getColumnKey(o);
      grupos[columnKey].push(o);
    });
    return grupos;
  }, [ordersForGrid]);
