    ...
    try {
      ...
      try {
        es = new EventSource((() => {
          ...
          if (isDev) return "/api/events/stream"
          const origin = (globalThis as any)?.location?.origin || ""
          const base = ((import.meta as any).env?.BASE_URL as string) || "/"
          const b = base.endsWith('/') ? base : base + '/'
          // Em prod, BASE_URL termina com /cardapio-digital-sistema/admin/
          // Precisamos subir um nível para /api/
          const apiPath = b.replace(/\/admin\/$/, '/api/')
          return `${origin}${apiPath}events/stream`
        })())
